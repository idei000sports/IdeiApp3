...
public class HomeFragment extends Fragment {
    ...
    public void getAllList() {
        ...
        String GET_RECENTLY_SQL = "SELECT song_title, album_art_path, id, song_path, artist_name from count_table ORDER BY id DESC;";
        getSQL(GET_RECENTLY_SQL, homeListMakerHashMap.get(R.id.recent_history), "song_title");

        String GET_THIS_MONTH_FAV_SQL = "SELECT album_title, album_art_path, count(*) AS COUNT, artist_name, song_path FROM count_and_date_table  WHERE date BETWEEN '" + oneMonthBefore + "' AND '" + today + "' GROUP BY artist_name, album_title ORDER BY COUNT DESC;";
        getHomeListInDate(GET_THIS_MONTH_FAV_SQL, homeListMakerHashMap.get(R.id.this_month_fav));

        String GET_MY_FAVORITE_SQL = "SELECT album_title, album_art_path, count(*) AS COUNT, artist_name, song_path FROM count_and_date_table GROUP BY artist_name, album_title ORDER BY COUNT DESC;";
        getHomeListInDate(GET_MY_FAVORITE_SQL, homeListMakerHashMap.get(R.id.my_favorite));

        String GET_PLAYBACK_LIST_SQL = "SELECT album_title, album_art_path, count(*) AS COUNT, artist_name, song_path FROM count_and_date_table  WHERE date BETWEEN '" + oneYearBefore + "' AND '" + today + "' GROUP BY artist_name, album_title ORDER BY COUNT DESC;";
            ...
            if (cursor != null && cursor.getCount() > 0) {
                ...
                while (next) {
                    if (i < 6) {
                        ...
                        list.get(i).setAlbumArtAndInfo(title, artistName, albumArtPath, songPath);

                        int finalI = i;
                        list.get(i).getImageButton().setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                Toast.makeText(getContext(), list.get(finalI).getArtistName() + list.get(finalI).getTitle(), Toast.LENGTH_SHORT).show();
                                //playSong(list.get(finalI).getSongPath(), list.get(finalI).getAlbumArtPath());

                                openAlbum(list.get(finalI).getTitle(), list.get(finalI).getArtistName());
    ...
    public void getSQL(String sql, ArrayList<HomeListBean> list, String wantColumnName) {
        String GET_RECENTLY_SQL = "SELECT song_title, album_art_path, song_path , artist_name FROM count_table ORDER BY id DESC;";
        SQLAlbumAndSongTableHelper sqlAlbumAndSongTableHelper = new SQLAlbumAndSongTableHelper(getActivity());
